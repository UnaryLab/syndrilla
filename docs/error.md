...
`rate` takes the swept range of §2 here too, regenerating the circuit at each level, so it needs the generation parameters `-i` passes through. See [interface.md](interface.md) §3.
