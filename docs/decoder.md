...
**Resuming (`-ckpt` and `-tckpt`).** The pair, `-ckpt <stem>_result.yaml` and `-tckpt <stem>_last.pt` with every other flag unchanged, continues a run to bit-identical weights and an identical curve; either alone is refused naming the other. State is restored from the `*_last.pt`, while the yaml is only checked: a run whose model, noise, schedule, `-bs`, loss or selection metric moved is refused, naming every field that did. A weights-only `*_best.pt` has no fingerprint and is refused pointing at `decoder.config.checkpoint`; `-tckpt` needs `-t`.
