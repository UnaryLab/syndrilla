...
Training writes `<stem>.pt`, `<stem>_last.pt`, `<stem>_history.json` and `<stem>_train.log` into `-r`, which defaults to `tests/test_outputs`, the same directory a decode run writes its results to; put the checkpoint path in `decoder.config.checkpoint` and evaluate with the normal CLI. Checkpoint I/O is driven by the metric module, which owns the run directory and the run position; the decoder supplies its own half of the state and owns the file format.
