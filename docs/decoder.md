...
decoder:
  ...
  device:
    ...
    device_idx: 0
  config:
    checkpoint: tests/test_outputs/saq_hx_d5.pt
    model:
      d_model: 128
      N_dec: 6
      h: 16
    ...
    train:
      ...
      val_batches: 20
      seed: 42
      # optional; absent, every epoch is written out
      epochs_saved: 20
```
...
| `decoder.config.checkpoint`         | (optional) Trained weights to load; a decode run needs it, `-t` writes it       | `tests/test_outputs/saq_hx_d5.pt` |
...
Training writes `<stem>.pt`, `<stem>_last.pt`, `<stem>_history.json`, `<stem>_result.yaml` and `<stem>_train.log` into `-r`, which defaults to `tests/test_outputs`, the same directory a decode run writes its results to; put the checkpoint path in `decoder.config.checkpoint` and evaluate with the normal CLI. Checkpoint I/O is driven by the metric module, which owns the run directory and the run position; the decoder supplies its own half of the state and owns the file format.

**`<stem>_result.yaml`.** The training counterpart of a decode run's `result_phy_err_<rate>.yaml`, and the same shape: a `train_full` block naming the run (algorithm, `model parameters` (the decoder's trainable weight count; the run's random seed is the separate `seed` key), device, dtype, error rate in the form it was configured, a swept range's point count included, batch size, schedule, seed, best epoch and best validation class error, what the run cost, checkpoint path), then the results themselves. The curve is stored **by column**: under `epoch` sit `epoch`, the list of epoch numbers, `learning rate`, `time (s)`, `best`, and a `train` and a `val` block holding one list per term the epoch line prints (`loss`, `lc`, `lp`, `ent`, `class error`). Every list is index-aligned with the epoch list, so `train['loss'][i]` and `val['class error'][i]` belong to epoch `epoch[i]`, and a term is one line to read and one list to plot rather than something to collect out of a block per epoch. The epoch numbers are carried explicitly rather than implied by position, which is what keeps the columns meaningful once `epochs_saved` has thinned them to a tail. It holds the same numbers as `<stem>_history.json` and is rewritten at every epoch boundary, so a run stopped part way still leaves behind the epochs it did finish:
...
The averages divide the summed *epoch* time rather than the wall clock, for two reasons. The wall clock includes building the decoder and loading the matrices, which is setup rather than training and would inflate a short run's per-epoch cost, and it is the time of this invocation alone, so a resumed run's wall clock covers only the epochs since the resume while its epoch times cover the whole run. A batch here is a batch of either phase, `batches_per_epoch + val_batches` of them per epoch, since both phases run through the same channel and the epoch time is the two together. Each epoch's own time is a column of the curve, so a run that slowed down partway shows it.

**Capping the curve (`epochs_saved`).** One entry per column per epoch is the right file for a hundred-epoch run and the wrong one for a hundred-thousand-epoch run, so `decoder.config.train.epochs_saved` bounds it: the two output files then carry the last `epochs_saved` epochs, plus the run's best epoch wherever it fell, since that is the epoch the saved checkpoint holds and `best epoch` names. The summary's `epochs` still reports the whole schedule, and `epochs saved` the cap itself, which appears only when one is set. `<stem>_history.json` honours the same cap: the two files are one record in two formats. Only the files are thinned, never `<stem>_last.pt`, so a resumed run restores the complete curve and the cap can be changed between runs at no cost. Leave the key out and every epoch is written, as before.
...
| `epoch`, `best`, `history` | `MetricState.train_state()` | where the run had got to, and which epoch was best |
