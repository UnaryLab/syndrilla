...
def build_pipeline(decoder_yaml, error_yaml, syndrome_yaml, logical_yaml,
...
def run_one_batch(metrics, decoders, em, sg, lc, bundle, batch_size,
                  number_channel, check_type, num_err):
    ...
    for err, llr, _ in dl:
        ...
        check = [lc.check(bt.e_v_all[i], bt.e_all, l_mat, bt.converge_all[i + 1])
                 for i in range(nd)]
        ...
        for i in range(nd):
            br = report_metric(nmi[i], bt.e_all, bt.e_v_all[i],
                               bt.iter_all[i], bt.time_iter_all[i],
                               check[i], bt.converge_all[i],
...
def snapshot(m, number_channel):
    ...
    snap = {}
    for field in ('logical_error_rate', 'data_qubit_acc', 'data_frame_error_rate',
                  'synd_frame_error_rate', 'correction_acc',
                  'converge_fail', 'converge_succ'):
        snap[field] = [[float(v) for v in row] for row in getattr(m, field)]
    for field in ('total_time', 'average_time_sample', 'average_iter',
...
def compare(snap, m_loaded, number_channel, num_decoders):
    ...
    for d in range(num_decoders):
        if not torch.equal(snap['distribution'][d].int(), m_loaded.distribution[d].int()):
            issues.append(('distribution', d, '-', '<tensor>', '<tensor>'))
    return issues


def run_one_config(label, decoder_yaml, error_yaml, syndrome_yaml, check_yaml,
                   check_type, tmp_dir, batch_size=200, save_every=20,
                   resume_extra=2, progress_every=5):
    print(f'\n========================================================================')
    ...
    target_error = 10**9
    rounds = getattr(sg, 'rounds', 1)
    print(f'  num_decoders={nd}  number_channel={nc}  rounds={rounds}  '
    ...
    for b in range(1, save_every + 1):
        t_b = time.time()
        ne = run_one_batch(m, decoders, em, sg, lc, bundle, batch_size, nc,
                           check_type, ne)
        if b % progress_every == 0 or b == 1 or b == save_every:
            print(f'    batch {b:>3d}/{save_every}  num_err={ne:>7d}  '
                  f'batch_time={time.time()-t_b:.3f}s  '
                  f'total={time.time()-t_start:.1f}s')

    out = m.get_all_metrics(save_every, algo)
    save_metric(out, tmp_dir + '/', batch_size, target_error, str(dtype),
                em.rate, save_every, ne, H_file, chk_num,
                vote_info=m.get_vote_info())
    ...
    m_loaded.validate_checkpoint(meta, batch_size, target_error, dtype,
                                 em.rate, H_file)
    ...
    post_avg = {f'd{i}': v for i, v in enumerate(
        m_loaded.get_all_metrics(meta['batch_count'], algo))}
    ...
    for d in range(nd):
        ...
        for field, pre_val in pre_avg[key].items():
            ...
            # tensor with shape (iteration distribution): summarize as equality
                ...
                eq = bool(torch.equal(pre_val.int() if hasattr(pre_val, 'int') else pre_val,
                                       post_val.int() if hasattr(post_val, 'int') else post_val))
                if not eq:
                    avg_issues.append((d, field, '<tensor>', '<tensor>'))
                print(fmt.format(f'd{d}.{field}', f'shape={tuple(pre_val.shape)}',
                                 f'shape={tuple(post_val.shape)}', eq))
    ...
    try:
        for b in range(1, resume_extra + 1):
            ne = run_one_batch(m_loaded, decoders, em, sg, lc, bundle, batch_size,
                               nc, check_type, ne)
            nb += 1
        print(f'    resumed OK — final batches={nb}  num_err={ne}  '
