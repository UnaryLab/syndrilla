...
TRAINING_DECODER_YAML = "examples/alist/train_saq_hx.decoder.yaml"
TESTING_DECODER_YAML = "examples/alist/saq_hx.decoder.yaml"
...
def _make_decoder(matrix_yaml, training=False, **overrides):
    ...
    cfg = read_yaml(get_path(TRAINING_DECODER_YAML))["decoder"]
    algo_cfg = cfg["config"]
...
def test_forward_matches_the_paper_pipeline():
    ...
    with torch.no_grad():
        ...
        assert SN.shape == (synd.size(0), saq.m + 1, 32)
...
def test_attention_mask_is_the_code_topology():
    ...
    assert torch.all(allowed[0, :]) and torch.all(allowed[:, 0])
    assert not torch.all(allowed)


def test_no_mask_ablation():
    """`no_mask: 1` must drop both attention masks."""
...
def test_entropy_loss_polarity(matrix_yaml):
    ...
    def ent(pred):
        """L_Ent for a prediction, decoded from a confident LLR."""
        io = {
...
def test_the_two_shipped_yamls_are_one_model_and_its_weights():
    """`train_saq_hx` and `saq_hx` must stay the same architecture, once trained."""
    train_cfg = read_yaml(get_path(TRAINING_DECODER_YAML))["decoder"]
    decode_cfg = read_yaml(get_path(TESTING_DECODER_YAML))["decoder"]
    assert "checkpoint" not in train_cfg["config"]

    stripped = _plain(decode_cfg)
    assert stripped["config"].pop(
        "checkpoint", None
    ), "the decode yaml names no weights"
    assert stripped == _plain(train_cfg)

    def build(cfg):
        """The decoder `cfg` describes, on the surface_5 matrix."""
        bundle = load_matrices(
    ...
    trained_wrapper, trained = build(decode_cfg)
    untrained_wrapper, untrained = build(train_cfg)
...
def test_decode_cli_refuses_a_learned_decoder_with_no_weights(tmp_path):
    """Decoding on the training yaml must stop the run rather than meter random weights."""
    result = _run_cli(
        tmp_path,
    ...
    assert "checkpoint" in result.stderr, result.stderr
    assert not list(tmp_path.glob("result_phy_err_*.yaml"))


def test_train_ignores_the_checkpoint_the_testing_yaml_names(tmp_path):
    """`-t` fits its own weights, so `checkpoint` must not warm-start it."""
    training_yaml = _write_training_yaml(
        tmp_path / "small.training.yaml", epochs=1, test_batches=2, validation_batches=1
    )
    trained = {}
    for name, decoder_yaml in (
        ("bare", TRAINING_DECODER_YAML),
        ("weighted", TESTING_DECODER_YAML),
    ):
        run_dir = tmp_path / name
        result = _run_cli(
            run_dir, *_train_argv(training_yaml, decoder_yaml=decoder_yaml)
        )
        assert result.returncode == 0, result.stderr
        trained[name] = torch.load(
            run_dir / BEST_PT, map_location="cpu", weights_only=True
        )

    for key, value in trained["bare"].items():
        assert torch.equal(trained["weighted"][key], value), key
    assert any("is ignored" in line for line in _run_log(tmp_path / "weighted"))
...
def _write_training_yaml(path, **overrides):
    """A copy of the shipped training yaml with a shrunk schedule, so tests stay fast."""
    cfg = _plain(read_yaml(get_path(TRAINING_YAML))["training"])
    cfg["schedule"] = dict(cfg["schedule"], **overrides)
...
def _trainer_for(decoder, epochs, **overrides):
    ...
    trainer = Trainer(
        {"optimizer": dict(_optimizer_cfg(), **overrides)}, _make_loss(decoder)
    )
...
def test_train_cli_writes_a_result_yaml_indexed_by_epoch(tmp_path):
    ...
    training_yaml = _write_training_yaml(
        tmp_path / "small.training.yaml", epochs=3, test_batches=4, validation_batches=2
    )
    result = _run_cli(
        tmp_path,
        "-t",
...
def test_train_cli_rejects_untrainable_and_missing_args(tmp_path):
    ...
    training_yaml = _write_training_yaml(
        tmp_path / "small.training.yaml", epochs=1, test_batches=1, validation_batches=1
    )
    ...
    not_trainable = _run_cli(
        tmp_path,
        "-t",
        "-d=examples/alist/mwpm_hx.decoder.yaml",
        f"-m={SURFACE_MATRIX_YAML}",
        f"-e={TRAIN_ERROR_YAML}",
        f"-s={TRAIN_SYNDROME_YAML}",
        f"-tr={training_yaml}",
...
def _write_bad_training_yaml(path, schedule):
    """A training yaml whose `schedule` block is `schedule`, or absent when it is None."""
    cfg = _plain(read_yaml(get_path(TRAINING_YAML))["training"])
    if schedule is None:
...
def test_train_cli_rejects_a_training_yaml_with_no_schedule(tmp_path):
    """`-t` on a training yaml carrying no `schedule` block must say so."""
    bad = _write_bad_training_yaml(tmp_path / "no_schedule.training.yaml", None)
    ...
    assert result.returncode != 0
    assert "no 'schedule' block" in result.stderr


def test_train_cli_rejects_a_schedule_missing_a_key(tmp_path):
    """A `schedule` block without every key must fail before any training."""
    shipped = read_yaml(get_path(TRAINING_YAML))["training"]["schedule"]
    bad = _write_bad_training_yaml(
        tmp_path / "bad.training.yaml",
        {k: v for k, v in shipped.items() if k != "validation_batches"},
    )
    ...
    assert result.returncode != 0
    assert "missing under 'training.schedule': validation_batches" in result.stderr


def test_train_cli_names_the_header_a_stale_loss_yaml_is_missing(tmp_path):
    """A yaml headed `loss:` under `-tr` must be told what it no longer is."""
    stale = tmp_path / "saq.loss.yaml"
    loss = _plain(read_yaml(get_path(TRAINING_YAML))["training"]["loss"])
    stale.write_text(yaml.safe_dump({"loss": loss}))
    ...
    assert result.returncode != 0
    assert "no `training` header" in result.stderr


def test_train_cli_requires_the_training_yaml(tmp_path):
    """`-t` without `-tr` must fail before anything is constructed."""
    result = _run_cli(
        tmp_path,
        "-t",
        f"-d={TRAINING_DECODER_YAML}",
...
def test_create_trainer_reads_the_shipped_yaml():
    """The factory must dispatch on `function` and pick up the lambdas from the yaml."""
    from syndrilla.trainer import create_trainer

    decoder, saq = _make_decoder(SURFACE_MATRIX_YAML)
    trainer = create_trainer(TRAINING_YAML, decoder=saq)
    loss = trainer.loss

    assert set(trainer.optimizer_cfg) == {"lr", "weight_decay", "min_lr"}
    assert trainer.schedule["epochs"] == 100
...
def test_train_metrics_take_back_epoch_best_and_history(tmp_path):
    ...
    metrics = MetricState.train_initial(cfg, str(tmp_path), TRAINING_YAML)
    trainer = _stub_trainer(cfg)
    metrics._decoder = trainer.model
    metrics._trainer = trainer
...
class _EpochDecoder:
    ...
    def __init__(self, score=None, score_name=None):
        """A decoder declaring the selection policy given, or none."""
    ...
    def train(self, training):
        """Accept the mode the run puts it in."""

    def state_dict(self):
        """Weights saying which epoch wrote them."""
        return {"w": torch.tensor([float(self.mark)])}


class _StubDecoder(torch.nn.Module):
    """A decoder with a mode and one weight: what the schedule tests need of one."""

    def __init__(self):
        """One weight, on the cpu."""
        super().__init__()
        self.weight = torch.nn.Parameter(torch.zeros(1))
        self.device = "cpu"


def _stub_trainer(cfg, decoder=None):
    """A real `Trainer` running to `cfg`'s schedule, over a decoder with nothing to fit."""
    from syndrilla.trainer import Trainer

    model = _StubDecoder() if decoder is None else decoder
    trainer = Trainer({"optimizer": _optimizer_cfg()}, _make_loss(model))
    trainer.configure(
        model,
        cfg["epochs"],
        cfg["test_batches"] + cfg["validation_batches"],
        cfg["test_batches"],
        cfg["error_random_seed"],
    )
    return trainer
...
def _run_epochs(tmp_path, decoder, epochs):
    ...
    metrics._trainer.decoder = decoder
    metrics._fingerprint = {"epochs": len(epochs)}
    metrics.batch_size = 8
    metrics.lr = 1e-3
    metrics._trainer.begin_batch(0, metrics.epoch)
    batch = 0
...
def test_resuming_under_a_different_selection_metric_is_refused(tmp_path):
    ...
    metrics = MetricState.train_initial(cfg, str(tmp_path), TRAINING_YAML)
    metrics._decoder = _EpochDecoder()
    metrics._fingerprint = {"epochs": 4}
...
def _metrics_under(tmp_path, term_names, **overrides):
    ...
    class _Loss:
        """A loss that is nothing but its `term_names`."""
    ...
    metrics._trainer = _stub_trainer(cfg)
    return metrics
...
def test_the_saq_loss_declares_every_term_it_returns():
    """The loss's `term_names` is the contract the metric half meters it by."""
    from syndrilla.trainer.saq import saq as saq_loss
    ...
    assert saq_loss.create.term_names == ("lc", "lp", "ent")
    assert len(loss.terms(decoder(_io_dict(saq, synd, H)), e)) == len(loss.term_names)


def _sequence(trainer, batch_index, epoch=1, k=6):
    """The first `k` draws of the phase `batch_index` opens."""
    trainer.begin_batch(batch_index, epoch)
    return torch.rand(k)
...
def test_every_epoch_trains_on_the_same_batches():
    """The training phase is a fixed set: epoch N draws what epoch 1 drew."""
    trainer = _stub_trainer(_SCHEDULE)
    period = _SCHEDULE["test_batches"] + _SCHEDULE["validation_batches"]

    first = _sequence(trainer, 0, epoch=1)
    third = _sequence(trainer, 3 * period, epoch=3)

    assert torch.equal(first, third)


def test_validation_draws_new_errors_each_epoch():
    """Validation is not the training set replayed, and not the same twice."""
    trainer = _stub_trainer(_SCHEDULE)
    period = _SCHEDULE["test_batches"] + _SCHEDULE["validation_batches"]

    val_1 = _sequence(trainer, _SCHEDULE["test_batches"], epoch=1)
    train = _sequence(trainer, 0, epoch=1)
    val_2 = _sequence(trainer, period + _SCHEDULE["test_batches"], epoch=2)
...
def test_begin_batch_reports_the_phase():
    """Seeding must not disturb which batches count as training."""
    trainer = _stub_trainer(_SCHEDULE)
    period = _SCHEDULE["test_batches"] + _SCHEDULE["validation_batches"]

    phases = [trainer.begin_batch(i, 1) for i in range(period * 2)]

    assert phases == ["train", "train", "val", "train", "train", "val"]
    assert trainer.phase == "val"


def test_begin_batch_puts_the_decoder_in_the_phase_it_opened():
    """The phase the trainer picks and the mode the decoder runs in are one decision."""
    decoder = _StubDecoder()
    trainer = _stub_trainer(_SCHEDULE, decoder)
    period = _SCHEDULE["test_batches"] + _SCHEDULE["validation_batches"]

    modes = []
    try:
        for i in range(period):
            trainer.begin_batch(i, 1)
...
def test_neighbouring_run_seeds_do_not_share_streams():
    """`error_random_seed` and `error_random_seed + 1` must not produce the same training set."""

    def train_draw(seed):
        """The first training batch a run seeded with `seed` draws."""
        return _sequence(_stub_trainer(dict(_SCHEDULE, error_random_seed=seed)), 0)
...
def _saq_block():
    """The shipped saq `config` block, shrunk so tests stay fast."""
    cfg = read_yaml(get_path(TRAINING_DECODER_YAML))["decoder"]["config"]
...
def test_train_cli_runs_a_chain_ending_in_the_trained_decoder(tmp_path):
    ...
    argv = _train_argv(training_yaml, decoder_yaml=chained)
    assert _run_cli(tmp_path, *argv).returncode == 0
...
def test_only_the_last_decoder_decides_whether_a_chain_can_train():
    ...
    def tail(algorithms, configs):
        """Whether the chain's last decoder can be trained."""
...
def test_train_cli_exits_when_the_last_decoder_cannot_train(tmp_path):
    ...
    argv = _train_argv(training_yaml, decoder_yaml=bad)
    assert _run_cli(tmp_path, *argv).returncode != 0


def _train_argv(training_yaml, *extra, decoder_yaml=TRAINING_DECODER_YAML):
...
def _interrupt_after_epoch(run_dir, training_yaml, epoch):
    """Start a training run and SIGINT it once `epoch`'s line reaches the run's log."""
    import signal
    import time

    cmd = ["syndrilla", f"-r={run_dir}", *_train_argv(training_yaml)]
...
def test_resume_cli_finishes_an_interrupted_run(tmp_path):
    ...
    training_yaml = _write_training_yaml(
        tmp_path / "small.training.yaml", epochs=4, test_batches=3, validation_batches=1
    )

    straight_dir = tmp_path / "straight"
    assert _run_cli(straight_dir, *_train_argv(training_yaml)).returncode == 0

    resumed_dir = tmp_path / "resumed"
    _interrupt_after_epoch(resumed_dir, training_yaml, 3)
...
def test_resume_rejects_a_changed_schedule(tmp_path):
    ...
    training_yaml = _write_training_yaml(
        tmp_path / "small.training.yaml", epochs=2, test_batches=2, validation_batches=1
    )
    assert _run_cli(tmp_path, *_train_argv(training_yaml)).returncode == 0

    changed = _write_training_yaml(
        tmp_path / "changed.training.yaml",
        epochs=2,
        test_batches=5,
        validation_batches=1,
    )
...
def test_resume_rejects_a_changed_setup(tmp_path):
    ...
    decoder_cfg = _plain(read_yaml(get_path(TRAINING_DECODER_YAML)))
    decoder_cfg["decoder"]["dtype"] = "float64"
    dtype_yaml.write_text(yaml.safe_dump(decoder_cfg))

    def rewritten(name, block, **changes):
        """The shrunk training yaml with one block changed."""
        cfg = yaml.safe_load(training_yaml.read_text())
        cfg["training"][block] = dict(cfg["training"][block], **changes)
        path = tmp_path / name
        path.write_text(yaml.safe_dump(cfg))
        return path

    reweighted = rewritten("reweighted.training.yaml", "loss", lambda_lp=0.5)
    restepped = rewritten("restepped.training.yaml", "optimizer", lr=1.0e-3)

    for field, override in (
        ("physical_error_rate", f"-e={error_yaml}"),
        ("dtype", f"-d={dtype_yaml}"),
        ("loss_lambda_lp", f"-tr={reweighted}"),
        ("optimizer_lr", f"-tr={restepped}"),
...
def test_resume_needs_both_checkpoints(tmp_path, given, missing):
    ...
    result = _run_cli(tmp_path, *_train_argv(training_yaml, f"{given}={path}"))
    assert result.returncode != 0
...
def test_resume_checks_the_result_yaml_it_was_given(tmp_path):
    ...
    other_training = _write_training_yaml(
        tmp_path / "other.training.yaml", epochs=2, test_batches=3, validation_batches=1
    )
    assert _run_cli(other_dir, *_train_argv(other_training)).returncode == 0
...
def test_resume_refuses_a_yaml_that_is_not_a_training_result(tmp_path):
    ...
    result = _run_cli(
        tmp_path,
        *_train_argv(
            training_yaml, f"-ckpt={decode_yaml}", f"-tckpt={tmp_path / LAST_PT}"
        ),
    )
    assert result.returncode != 0
...
def test_the_run_describes_its_model_in_the_resume_fingerprint():
    ...
    _, saq = _make_decoder(SURFACE_MATRIX_YAML)
    trainer = _trainer_for(saq, 4)
    model = trainer.train_fingerprint()
    assert model == {
        "algo": "saq",
        "n": saq.n,
        "m": saq.m,
    ...
    trainer.loss = _make_loss(saq)
    merged = _train_fingerprint(
        cfg, 16, [0.01, 0.20, 9], "/codes/surface_5.hx.alist", trainer
    )
    ...
    assert merged["loss_function"] == "saq"
    assert merged["loss_lambda_lp"] == trainer.loss.lambda_lp
    assert merged["optimizer_lr"] == trainer.optimizer_cfg["lr"]


def test_decoder_reads_its_settings_from_their_own_blocks():
    """Architecture and CPND settings each come from their own block."""
    _, saq = _make_decoder(
        SURFACE_MATRIX_YAML,
    ...
    assert saq.N_dec == 3
    assert saq.src_mask_SN is None
...
def test_a_decoder_yaml_still_carrying_the_run_settings_is_rejected(block, moved):
    """The run's own settings left in `decoder.config` must name where they went."""
    with pytest.raises(ValueError, match=moved.replace(".", r"\.")):
        _make_decoder(SURFACE_MATRIX_YAML, **{block: {"lr": 1.0e-3}})


def test_the_optimizer_settings_come_from_the_training_yaml():
    """The trainer fits the decoder from its own block, not from anything the model holds."""
    _, saq = _make_decoder(SURFACE_MATRIX_YAML)
    trainer = _trainer_for(saq, 2, lr=1.0e-3, weight_decay=2.0e-7, min_lr=3.0e-6)

    group = trainer.optimizer.param_groups[0]
    assert (group["lr"], group["weight_decay"]) == (1.0e-3, 2.0e-7)
    assert trainer.scheduler.eta_min == 3.0e-6
...
def test_training_mode_turns_cpnd_off_in_the_decoder():
    ...
    assert not hasattr(trained, "cpnd_supports")
...
def test_train_state_carries_the_random_state():
    ...
    rng = _trainer_for(cpu_saq, 2).train_state()["rng"]
    ...
    if torch.cuda.is_available():
        ...
        state = _trainer_for(gpu_saq, 2).train_state()
