...
def get_stim_circuit(circuit_str=None, circuit_cfg=None):
    ...
    if circuit_str is not None and not isinstance(circuit_str, dict):
        ...
        gen_kwargs = {}
        for noise_key in ('after_clifford_depolarization', 'after_reset_flip_probability',
                          'before_measure_flip_probability', 'before_round_data_depolarization'):
            if noise_key in cfg:
                gen_kwargs[noise_key] = float(cfg[noise_key])
        logger.info(f'Generating stim circuit: {code}, distance={distance}, rounds={rounds}.')
        return stim.Circuit.generated(code, distance=distance, rounds=rounds, **gen_kwargs)
    else:
...
class create():
    ...
    def __init__(self, interface_cfg, **kwargs) -> None:
        ...
        if decoder_cfg:
            ...
            device_cfg = decoder_cfg.get('device', {'device_type': device.type,
        ...
        number_channel = error_cfg.get('number_channel', interface_cfg.get('number_channel', 1))
        ...
        if isinstance(circuit_inline, dict):
            ...
            noise_keys = ('after_clifford_depolarization', 'after_reset_flip_probability',
                          'before_measure_flip_probability', 'before_round_data_depolarization')
            circuit_gen_cfg = {
        ...
        self.logical_check = create_check(cfg={'check_type': 'stim'})
        ...
        if decoder_cfg and 'algorithm' in decoder_cfg:
            self.decoders = create_decoder(cfg=decoder_cfg, bundle=self.matrix_bundle)
        else:
            self.decoders = None
